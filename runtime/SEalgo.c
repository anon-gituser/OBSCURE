...
static void LLS_decrypt(u8 *lls_bytecode, AELLS *aells, u8 *shared_key, u32 lb_m, u32 lb_o)
{
  ...
  u8 *ptr = ad + U32_INBYTES + FLAG_INBYTES + lb_m;
  for(i=0; i<aells->inp_count; i++){
    u32_tobytes(ptr, U32_INBYTES, aells->inputIDs[i].instrID); ptr += U32_INBYTES;
    u32_tobytes(ptr, lb_o, aells->inputIDs[i].outputID); ptr += lb_o;
  }
  u32_tobytes(ptr, lb_m, aells->out_count); ptr += lb_m;
...
static void instruction_execute(uSE *ret, u8 opcode, uSE operand1, uSE operand2){
  uSE result;
  switch (opcode){
    case MOV:
    ...
    case LSR:
      ...
      break;
    case ASR:
      fprintf(stderr, "Operation not implemented yet: ASR.\n");
      exit(EXIT_FAILURE);
      break;
    case ADD:
      result = operand1 + operand2;
    ...
    case MUL:
      ...
      break;
    case ADDC:
      fprintf(stderr, "Operation not implemented yet: ADDC.\n");
      exit(EXIT_FAILURE);
      break;
    case SUBC:
      fprintf(stderr, "Operation not implemented yet: SUBC.\n");
      exit(EXIT_FAILURE);
      break;
    case DIV:
      result = operand1 / operand2;
                        ...
                        u32 lb_r)
{
  ...
  while (ptr != lls_bytecode + lls_bytelen){
    ...
    u8 src1_flag = (*ptr & 0x0C) >> 2; // next 2 bits
    u8 src2_flag = (*ptr & 0x03);      // last 2 bits
    ptr++;
    ...
    switch (src1_flag){
      case REG_FLAG:
        src1 = load_u32(ptr, lb_r);
        val1 = reg[src1];
        ptr += lb_r;
        break;
      case IMM_FLAG:
        val1 = load_word(ptr, WORD_INBYTES);
        ptr += lb_c;
        break;
      default:
        fprintf(stderr, "Invalid flag src1: %d.\n", src1_flag);
        exit(EXIT_FAILURE);
    }
    ...
    switch (src2_flag){
      case NUL_FLAG:
        break;
      case REG_FLAG:
        src2 = load_u32(ptr, lb_r);
        val2 = reg[src2];
        ptr += lb_r;
        break;
      case IMM_FLAG:
        val2 = load_word(ptr, lb_c);
        ptr += lb_c;
        break;
      default:
        fprintf(stderr, "Invalid flag src2: %d.\n", src2_flag);
        exit(EXIT_FAILURE);
    }

    instruction_execute(&ret, opcode, val1, val2);
    reg[dst] = ret;
  }
}
